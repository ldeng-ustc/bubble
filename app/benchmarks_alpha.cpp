...
int main(int argc, char** argv) {
    ...
    options.add_options()
        ...
        ("a,alpha", "Merging Efficiency threshold alpha", cxxopts::value<double>()->default_value("2.0"))
        ;
    ...
    config.merge_multiplier = result["alpha"].as<double>();
    ...
    //     g->AddEdge(RawEdge32<void>{edge_buffer[i].from, edge_buffer[i].to});
    // }
    ...
    //     if(tid == 0) {
    //         for(size_t i = 0; i < edge_count; i++) {
    //             g->AddEdgeIn(RawEdge32<void>{edge_buffer[i].from, edge_buffer[i].to});
    //         }
    //     } else {
    //         for(size_t i = 0; i < edge_count; i++) {
    //             g->AddEdgeOut(RawEdge32<void>{edge_buffer[i].from, edge_buffer[i].to});
    //         }
    //     }
    // }
    ...
    //     for(size_t i = 0; i < edge_count; i++) {
    //         // fmt::println("WAdd({}): {}", tid, RawEdge32<void>{edge_buffer[i].from, edge_buffer[i].to});
    //         g->AddEdgeMultiThread(RawEdge32<void>{edge_buffer[i].from, edge_buffer[i].to}, tid);
    //     }
    // }
    ...
    edge_buffer.reset();
    timer.Lap();
    // g->IterateNeighborsOut(1, [](VID32 v) {
    //     fmt::println("{}", v);
    // });

    // g->OutGraph().IterateNeighborsInOrder(1, [](VID32 v) {
    //     fmt::println("{}", v);
    // });
    // exit(0);
