...
int main() {
    ...
    auto [rt, pt] = ScanLargeFile<RawEdge64<void>, 8*1024*1024>(dataset, [&](RawEdge64<void> e) {
        g->AddEdge(RawEdge32<void>{e.to, e.from});
    });
    ...
    std::unique_ptr<NodeID []> comp;
    auto t = TimeIt([&] {
        comp = connected_components(g.get(), false);
    });
    g->FinishAlgorithm();
    
    fmt::println("CC time: {:.2f}s", t);
    fmt::println("Printing component stats ...");
    PrintCompStats(std::span<const NodeID>(comp.get(), g->VertexCount()));
    return 0;
}