...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <dataset> <vertex_count> <stdout_file> <ingest_thread> <sort_batch_size>"
    echo "Example: $0 data/friendster.bin 69000000 ./tmp 80 128"
...
pushd "$(dirname "$script_file")"/../..
    ...
    # Run the benchmark
    pushd .
        ./build/app/benchmarks_ordered --b32 -t $ingest_thread -u $sort_batch_size -f $DATASET_FILE 2>&1 > $TMP_OUTPUT_FILE
