...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <dataset> <vertex_count> <stdout_file> <ingest_thread> <alpha>"
...
pushd "$(dirname "$script_file")"/../..
    ...
    # Run the benchmark
    pushd .
        stdbuf -oL -eL ./build/app/benchmarks_alpha --b32 -t $ingest_thread -f $DATASET_FILE -a $alpha > $TMP_OUTPUT_FILE 2>&1
